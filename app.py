# =====================================================
# OPSI A PRO v4.1 — PRODUCTION (SL SUPPORT-BASED)
# =====================================================
...
def get_liquid_symbols(min_vol):
    ...
    try:
        ...
        for d in r.json().get("data", []):
            if d["instId"].endswith("-USDT") and float(d.get("volCcy24h",0)) >= min_vol:
                symbols.append(d["instId"])
    except:
        pass
    ...
    try:
        ...
        for d in r.json().get("data", []):
            s = d.get("symbol","")
            if s.endswith("USDT") and float(d.get("baseVolume24h",0)) >= min_vol:
...
# =====================
# SUPPORT DETECTION (FIX SL)
# =====================
def find_support(df, lb=SR_LOOKBACK):
    supports=[]
...
def check_signal(okx, symbol):
    ...
    supports = [s for s in find_support(df1d) if s < entry]
    if not supports:
        return None
...
def render_chart(okx, signal):
    df = pd.DataFrame(okx.fetch_ohlcv(signal["Symbol"], ENTRY_TF, limit=120),
        columns=["t","open","high","low","close","volume"])
...
st.title("🚀 OPSI A PRO v4.1 — SL SUPPORT-BASED")
...
with tab2:
    st.dataframe(
        load_signal_history().sort_values("Time",ascending=False),
